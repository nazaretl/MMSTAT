...
# Book:         Test Book
# ------------------------------------------------------------------------------
# Quantlet:     MMSTAThelper_function
# ------------------------------------------------------------------------------
# Description:  This quantlet gives needed functions for quantlets including
#               MMSTATassociation and MMSTATconfmean 
# ------------------------------------------------------------------------------
# Datafiles:    -
# ------------------------------------------------------------------------------
# Inputs:       -
# ------------------------------------------------------------------------------
# Output:       Loading needed functions in memory for sourcing.     
# ------------------------------------------------------------------------------
# Example:      See MMSTATassociation and MMSTATconfmean.
# ------------------------------------------------------------------------------
# See also:     MMSTATassociation, MMSTATconfmean
# ------------------------------------------------------------------------------
# Keywords:     distribution, scatterplot, plot, mean, confidence interval,
#               association, contingency table, discrete, chisquare 
# ------------------------------------------------------------------------------
# Author:       Yafei Xu
# ------------------------------------------------------------------------------
        ...
        if (is.null(ret[[name]]) || (length(ret[[name]]) == 0)) {
            if (is.null(local[[name]])) {
                stopif(is.null(mmstat$UI[[name]]$value) && 
				               is.null(mmstat$UI[[name]]$selected), 
                               paste0("mmstat.getValues: 
							          no default value(s) 
									  for \"", 
                                      name, "\""))
...
mmstat.attrVar = function(var, type, index = NULL) {
    ret = var
    if (is.null(index)) 
        index = 1:length(var$values) else {
    ...
    if (type == "numvars") {
        if (is.null(var$jitter)) 
            ...
            ret$quart  = quantile(var$values[index], c(0.25, 0.75), 
		                     na.rm = T)
            ret$min    = min(var$values[index], na.rm = T)
            ret$max    = max(var$values[index], na.rm = T)
    }
    if ((type == "binvars") || (type == "ordvars") || 
	    (type == "facvars")) {
...
mmstat.getVar = function(dataname = NULL, varname = NULL, vartype = NULL, 
    ...
    deftype = c("numvars", "binvars", "ordvars", "facvars", "numeric", 
        "binary", "ordered", "factor")
    type = pmatch(vartype, deftype)
    stopif(is.na(type), paste0("mmstat.getVar: Unknown variable type: ", 
        vartype))
    type = deftype[type]
    if (type == "numeric") 
        type = "numvars"
...
mmstat.html = function(file, ...) {
    stopif(!file.exists(file), sprintf("File '%s' does not exist", 
	       file))
    html = paste0(readLines(file), collapse = "")
    stopif(!is.ASCII(html), sprintf("File '%s' contains non-ASCII symbols", 
        file))
    args = list(...)
...
mmstat.plotTestRegions = function(crit, xlim, ylim, cex, close = F, 
    ...
    text((co + cu)/2, mean(ylim), mmstat.math("\\\"&H[0];\\\""), 
	     cex = cex, 
         col = col)
    if (!is.null(text)) {
        if (pos == 2) 
            text(xlim[1], mmstat.pos(ylim, -0.25), label, col = col, 
			cex = cex, 
            pos = 4)
        if (pos == 4) 
            text(xlim[2], mmstat.pos(ylim, -0.25), label, col = col, 
			cex = cex, 
            pos = 2)
    }
}
...
mmstat.htest = function(...) {
    ...
    htest = list(method = list(attr = NULL, names = "", fmt = "%s", 
	             lines = 0), 
        alternative = list(attr = NULL, names = "Alternative:", 
		                   fmt = "%s", 
                           lines = 0), 
			null.value = list(attr  = "names", 
			                  names = vector("character", 0), 
							  fmt = "%.4f", lines = 1), 
							  data.name = list(attr = NULL, 
                                               names = "Data:", 
											   fmt = "%s", 
											   lines = 0), 
			estimate = list(attr = "names", 
                            names = vector("character", 0), 
							fmt = "%.4f", 
							lines = 0), 
    ...
    for (j in seq(nhtest)) {
        ...
        for (i in seq(tests)) {
            ...
            if (!is.null(telem)) {
                if (is.null(attr)) {
                  htest[[name]]$tab[1, i] = sprintf(htest[[name]]$fmt, 
                  telem)
                  ...
                  htest[[name]]$tab[match(as.character(attr(telem, attr)), 
                  htest[[name]]$names), i] = sprintf(htest[[name]]$fmt, 
                  telem)
                }
            }
        }
    ...
    for (j in seq(tab)) {
        if (j <= nrow(tab)) 
            tab[j] = sprintf("%-*s", maxlen[1 + ((j - 1)%/%nrow(tab))], 
                             tab[j]) else tab[j] = sprintf("%*s", 
				                                           maxlen[1 + 
											               ((j - 1)%/%
														   nrow(tab))], 
                                                           tab[j])
...
mmstat.ui.elem = function(inputId, type, ...) {
    ...
    if (!is.na(pos)) {
        ...
        if (elem$type == "sampleSize") {
            # sliderInput for significance level
            if (is.null(elem$label)) 
                ...
                elem$ticks  = T
            if (is.null(elem$value)) 
                elem$value = as.numeric(compareVersion(mmstat$shiny, 
				                                       "0.11") < 
                  0)
        }
        ...
        if (elem$type == "testHypotheses") {
            # radiobuttons for two and one-sided tests
            elem$call = "radioButtons"
            if (is.null(elem$label)) 
                elem$label = gettext("Choose test type")
            elem$choices = gettext(c("two.sided", "less", 
			                         "greater"), "name")
            if (is.null(elem$value)) 
                elem$value = "two.sided"
        }
        if (elem$type == "significance") {
            # special slider for significance level
            elem$call   = "mmstat.sliderInput"
            elem$update = "updateSliderInput"
            if (is.null(elem$ticks)) 
                elem$ticks = c(0.1, 0.25, 0.5, 1, 2, 5, 10, 20)
            if (is.null(elem$label)) 
            ...
            if (is.null(elem$value)) 
                elem$value = 6 - as.numeric(compareVersion(mmstat$shiny, 
                  "0.11") >= 0)
        }
        if (elem$type == "confidenceLevel") {
            # special slider for confidence level
            elem$call   = "mmstat.sliderInput"
            elem$update = "updateSliderInput"
            if (is.null(elem$ticks)) 
                elem$ticks = c(80, 85, 90, 95, 98, 99, 99.5, 99.9)
            if (is.null(elem$label)) 
                elem$label = HTML(gettext("Select confidence level 
				                           (1-&alpha;)"))
            elem$step = 1
            elem$min  = 1
            elem$max  = length(elem$ticks)
            if (is.null(elem$value)) 
                elem$value = 4 - as.numeric(compareVersion(mmstat$shiny, 
                  "0.11") >= 0)
        }
        if (elem$type == "dataSet") {
            # selectInput to select between datasets
        ...
        if (elem$type == "variable1") {
            # selectInput to select one variable
            elem$call = "selectInput"
            if (is.null(elem$label)) 
            ...
            if (is.null(elem$value)) 
                elem$value = mmstat.getVarNames(1, elem$vartype, 1)
        }
        ...
        if (elem$type == "fontSize") {
            # sliderInput for choosing font size
            elem$call   = "mmstat.sliderInput"
            elem$update = "updateSliderInput"
            if (is.null(elem$label)) 
...
mmstat.lang = function(deflang = NULL) {
    ...
    if (is.null(deflang)) {
        lang = sapply(strsplit(pof, ".", fixed = T), function(elem) {
            elem[1]
        })
        pat  = paste0("_", lang, "$")
        path = getwd()
    ...
    mmstat.warn(!file.exists(msgfn), sprintf("File '%s' does not exist", 
                                             msgfn))
...
mmstat = list(debug = 1, shiny = sessionInfo()$otherPkgs$shiny$Version, 
