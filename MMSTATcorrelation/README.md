...
## [<img src="https://github.com/QuantLet/Styleguide-and-FAQ/blob/master/pictures/qloqo.png" alt="Visit QuantNet">](http://quantlet.de/) **MMSTATcorrelation** [<img src="https://github.com/QuantLet/Styleguide-and-FAQ/blob/master/pictures/QN2.png" width="60" alt="Visit QuantNet 2.0">](http://quantlet.de/)
...
Description : 'Shows either a 2D or 3D scatterplot and computes Pearson's correlation and
Spearman's rank correlation for variables of the data sets CARS, DECATHLON and USCRIME.'

Keywords : 'plot, scatterplot, correlation, data visualization, visualization, estimation,
parameter, interactive, uscrime, 3D'

See also : 'BCS_Linreg, SFE5dim, MVAregbank, COPcorrelation1, MMSTATtime_series_1, MMSTATlinreg,
MMSTATconfmean, MMSTATconfi_sigma, MMSTATassociation, MMSTAThelper_function, MMSTATcorrelation'
...
Example : 'Shows the 2-dimensional scatterplot for the variables TOTALAREA and POPULATION in the
data set USCRIME'

```

![Picture1](MMSTATcorrelation.png)
...
mmstat.ui.elem("graph", "radioButtons", 
               ...
               choices  = gettext(c("2D.SCATTERPLOT", 
                                    "3D.SCATTERPLOT", 
                                    "SCATTERPLOTMATRIX"), 
                                  "name"), 
...
mmstat.ui.elem("coeff", "checkboxGroupInput", 
               ...
               choices = gettext(c("SHOW.BRAVAIS.PEARSON", 
                                   "SHOW.SPEARMAN"), "name"), 
               value   = character())
...
buildList = function(depth, v) {
  if (depth) {
    ...
    ind = apply(mat, 1, function(x, m) {length(unique(x)) == m}, 
      m = depth + 1)
    return(mat[ind, ])
  }
  ...
  output$scatterPlot = renderPlot({
    ...
    if (nrow(clist) == 0) {
      plot(0, 0, 
           ...
           cex  = 0.5)
      text(0, 0, 
           gettext("Please select two, three or more variables!"), 
           cex = 1.5 * inp$cex)
    } else {
      ...
      ct <<- ct%%nrow(clist)
      varx = mmstat.getVar(isolate(inp$dataset), 
                           varname   = clist[ct + 1, 1], 
                           na.action = na.pass)
      if (varx$name == clist[ct + 1, 1]) {
        ...
        if (inp$graph == "3D.SCATTERPLOT") {
          ...
          varz = mmstat.getVar(isolate(inp$dataset), 
                               varname   = clist[ct + 1, 3], 
                               na.action = na.pass)
          scatterplot3d(varx$values, 
                        vary$values, 
                        varz$values, 
                        pch          = 19, 
                        cex          = 0.75, 
          ...
          if (length(inp$coeff)) 
            title(main = gettext("Coefficients are not 
                                 available for 3D Scatterplot"))
        }
        if (inp$graph == "SCATTERPLOTMATRIX") {
        ...
        axis.text.cex = ifelse(ncol(dat) < 5, 
        inp$cex/(ncol(dat)^(1/3)), 0), 
        coeff         = inp$coeff, 
...
buildList = function(depth, v) {
  if (depth) {
    ...
    ind = apply(mat, 1, function(x, m) { length(unique(x)) == m}, m = depth+1)
    return(mat[ind,])
  }
  return(as.matrix(v, ncol = 1))
}
...
server = shinyServer(function(input, output, session) {
  ...
  output$scatterPlot = renderPlot({
    ...
    if (nrow(clist) == 0) {
      ...
      if (inp$permute>last.permute) { # button was pressed
        last.permute <<- inp$permute
        ct <<- ct+1
      }
      ct <<- ct%%nrow(clist)
      ...
      if (varx$name == clist[ct+1,1]) {
        ...
        if (inp$graph == '3D.SCATTERPLOT') {
          ...
          scatterplot3d (varx$values, 
                         ...
                         ylab         = gettext(clist[ct+1,2]), 
                         zlab         = gettext(clist[ct+1,3]))
          if (length(inp$coeff)) title(main = gettext("Coefficients are 
                                                      not available for 
                                                      3D Scatterplot"))
        }
        if (inp$graph == 'SCATTERPLOTMATRIX') {
                ...
                panel = function(x, y, i, j, cex, coeff,  cex.coeff, ...) {
                  if (length(coeff)) {
                    if(i>j) {
                      ...
                      xpos = mean(range(x))
                      ypos = min(y)+diff(range(y))*c(1,3,5)/(2*length(coeff))
                      for (i in seq(main)) panel.text(xpos, ypos[i], 
                                                      mmstat.math(main[i]), 
                                                      cex = cex.coeff)
                    }
                  } else {
