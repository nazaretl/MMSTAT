...
proportion which
uses a normal approximation. The user can interactively choose the test type (two sided, less or
greater),
the hypothetical proportion, the significance level (in %) and the sample size. Also, a table with
summary
statistics regarding the sample, population and the test is given. The sample can be drawn for
variables
of the data sets BOSTON HOUSING and CREDIT.'
